...
const filterData = (data: any, codes: any) => {
  const result = []
  for (const key in data) {
    const filteredValues = data[key].filter((value: any) =>
...
function ModalZone({
  ...
  const filteredData = filterData(groupedData, checkedData)
  const [listChecked, setListChecked] = useState<any>(filteredData)

  useEffect(() => {
    setListChecked(filteredData)
  }, [checkedData])
      ...
      >
        <Box>
          {sortedKeys.map(letter => (
            <Box key={letter} mb="sm" className="pb-2 border-b-2 px-4">
